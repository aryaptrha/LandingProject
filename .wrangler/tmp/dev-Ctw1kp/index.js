...
// src/worker/services/edge.service.ts
function getCf(request) {
  return request.cf ?? {};
}
__name(getCf, "getCf");
function getEdgeStatus(request) {
  const cf = getCf(request);
  return {
    status: "online",
    server: "Cloudflare",
    colo: ("colo" in cf ? cf.colo : void 0) ?? "unknown",
    country: ("country" in cf ? cf.country : void 0) ?? "unknown",
    countryCode: ("country" in cf ? cf.country : void 0) ?? "unknown",
    city: ("city" in cf ? cf.city : void 0) ?? "unknown",
    continent: ("continent" in cf ? cf.continent : void 0) ?? "unknown",
    timezone: ("timezone" in cf ? cf.timezone : void 0) ?? "unknown",
    protocol: ("httpProtocol" in cf ? cf.httpProtocol : void 0) ?? "unknown",
    tlsVersion: ("tlsVersion" in cf ? cf.tlsVersion : void 0) ?? "unknown",
    ray: request.headers.get("cf-ray") ?? "unknown",
    cacheStatus: request.headers.get("cf-cache-status") ?? "NONE",
    timestamp: (/* @__PURE__ */ new Date()).toISOString()
  };
}
__name(getEdgeStatus, "getEdgeStatus");
function getLatency(request) {
  const cf = getCf(request);
  return {
    timestamp: (/* @__PURE__ */ new Date()).toISOString(),
    server: "Cloudflare",
...
function error(message, code, status = 500) {
  const body = {
    success: false,
    error: { message, code }
  };
...
// src/worker/routes/latency.ts
var latency = new Hono2();
latency.get("/latency", (c) => {
  const data = getLatency(c.req.raw);
  return success(data);
});

// src/worker/routes/visitor.ts
var visitor = new Hono2();
visitor.get("/visitor", (c) => {
  const data = getVisitor(c.req.raw);
  return success(data);
});

// src/worker/routes/cache.ts
var cache = new Hono2();
cache.get("/cache", (c) => {
...
var chat = new Hono2();
function resolvePersonaUrl(raw2) {
  const clean = raw2.trim().replace(/\/$/, "");
  return clean.endsWith("/api/chat") ? clean : `${clean}/api/chat`;
}
__name(resolvePersonaUrl, "resolvePersonaUrl");
    ...
    if (envApiUrl && envApiUrl.trim()) {
      ...
      try {
        ...
        if (extContentType.includes("application/json")) {
          ...
          return c.json(extJson, extRes.status);
        }
        const extText = await extRes.text();
        return c.json({ success: true, reply: extText }, extRes.status);
      } catch (proxyErr) {
...
// .wrangler/tmp/bundle-zCH4Yw/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-zCH4Yw/middleware-loader.entry.ts
